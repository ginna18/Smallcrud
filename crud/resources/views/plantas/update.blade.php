    ...
    <body class="container p-3">
        ...
        <nav>
            <ul class="nav nav-pills my-3">
                ...
                <li class="nav-item mr-2">
                    <a class="nav-link" href="{{route('plantas.index')}}">HUERTO</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="{{route('plantas.create')}}">Nueva Planta</a>
                </li>
            </ul>
        </nav>
        <!-- fin menu nav-->
        <!--parte central-->
        <h1 class="my-2"> segundo ejemplo con laravel</h1>
        <main>
            <h2>Actualizacion de la planta{{"$planta->categoria $planta->tipo"}}</h2>
            @if ($errors->any())
            ...
            <form class="my-2 border p-5" method="POST" action="{{route('plantas.update',$planta->id)}}">
                @csrf
                <input name="_method" type="hidden" value="PUT">

                <div class="form-group row">
                    <label for="inputCategoria" class="col-sm-2 col-form-label">categoria</label>
                    <input name="categoria" value="{{$planta->categoria}}" type="text"
                    class="up form-control col-sm-10" id="inputCategoria"
                    placeholder="Categoria" maxlength="255" required>
                </div>

                <div class="form-group row">
                    <label for="inputipo" class="col-sm-2 col-form-label">TIPO</label>
                    <input name="tipo" value="{{$planta->tipo}}" type="text"
                    class="up form-control col-sm-10" id="inputipo"
                    placeholder="Tipo" maxlength="255" required>
                </div>

                <div class="form-group row">
                    <label for="inputNombres" class="col-sm-2 col-form-label">NOMBRE SILVESTRE</label>
                    <input name="nombres" value="{{$planta->nombres}}" type="text"
                    class="up form-control col-sm-10" id="inputNombres"
                    placeholder="Nombres" maxlength="255" required>
                </div>

                <div class="form-group row">
                    <label for="inputNombrec" class="col-sm-2 col-form-label">NOMBRE CIENTIFICO</label>
                    <input name="nombrec" value="{{$planta->nombrec}}" type="text"
                    class="up form-control col-sm-10" id="inputNombrec"
                    placeholder="Nombrec" maxlength="255" required>
                </div>

                <div class="form-group row">
                    <label for="inputDescripcion" class="col-sm-2 col-form-label">DESCRIPCION</label>
                    <input name="descripcion" value="{{$planta->descripcion}}" type="text"
                ...
                <div class="col-sm-9">
                    <label for="inputImagen" class="col-sm-2 col-form-label">
                        {{$planta->imagen? 'Sustituir':'Añadir'}} imagen
                    </label>
                    <input name="imagen" type="file"
                    class="form-control-file" id="inputImagen">
                    @if($planta->imagen)
                    ...
                    <script>
                        inputEliminar.onchange =function(){
                            inputImagen.disable =this.checked;
                        }
                    </script>
                    @endif
                </div>
            ...
            <div class="text-end my-3">
                <div class="btn-group mx-2">
                    <a class="mx-2" href="{{route('plantas.show',$planta->id)}}">
            ...
            <div class="btn-group" role="group" aria-label="links">
                ...
                <a href="{{route('plantas.index')}}" class="btn btn-primary m-2"> Guardar</a>
     ...
     <!--    <footer class="page-footer font-small p-4 bg-light">
            <p>aplicacion imitada por ginger navas como ejemeplo de clase.
